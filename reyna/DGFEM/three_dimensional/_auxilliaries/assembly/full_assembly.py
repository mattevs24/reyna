...
from reyna.DGFEM.three_dimensional._auxilliaries.assembly_aux import reference_to_physical_t4, \
...
def localstiff(nodes: np.ndarray,
    ...
    # TODO: Confirm that this generalises and works as expected -- note for d=1 too.
    B = 0.5 * nodes[1:, :] - nodes[0, :]
    De_tri = np.abs(np.linalg.det(B))

    weights = De_tri * weights

    P_Qpoints = reference_to_physical_t4(nodes, ref_points)  # Map reference to physical points
    ...
    h = 0.5 * (bounding_box[1::2] - bounding_box[::2])
    m = 0.5 * (bounding_box[1::2] + bounding_box[::2])
    ...
    if diffusion is not None:
        # TODO: this should work as expected but need to confirm
...
def int_localstiff(nodes: np.ndarray,
    ...
    # TODO: This is going to change to be more similar to the 2D element triangle transformation
    #  rewrite reference_to_element to admit this?
    ...
    # TODO: Check these are as expected
    h1 = 0.5 * (bounding_box1[1::2] - bounding_box1[::2])
    m1 = 0.5 * (bounding_box1[1::2] + bounding_box1[::2])
    h2 = 0.5 * (bounding_box2[1::2] - bounding_box2[::2])
    m2 = 0.5 * (bounding_box2[1::2] + bounding_box2[::2])
    ...
    if diffusion is not None:
        ...
        # TODO: this here will be rewritten in a nasty nasty way as have to consider more here with the simplicial
        ...
        # TODO: hopefully everything from here will be the same in diffusion part?
    ...
    if advection is not None:
        ...
        # TODO: this should all be the same here I think -- there is not reliance on the dimension of the space at all.
