...
class BoundaryInformation:
    ...
    def plot_boundaries(self) -> None:
        ...
        # TODO: need to consider this plotting function in 3D! This should be simple ish -- just time consuming.

        fig, ax = plt.subplots()

        if self.elliptical_indecies is not None:
            for i, ind in enumerate(self.elliptical_indecies):
                edge_vertices = self.geometry.nodes[self.geometry.boundary_facets[ind, :], :]
